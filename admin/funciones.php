<?php
    ...
    public $modifBlends;
		...
		public function altaBlends($blend,$precio,$descripcion){
				...
				$resultado = $this -> mysqli->query("INSERT into newblends(id, blend, precio, descripcion, imagenBlend) VALUES('0','$blend','$precio','$descripcion', '$idImagenUnico')"); 
		...
		//=======================================================================//		
			...
			public function listadoBlends(){
				$sql="SELECT * FROM newblends LEFT OUTER JOIN imagenes ON newblends.imagenBlend = imagenes.idCodigoImg";
    ...
    public function modificarBlends($idblend, $idCodImg){ 
   
    $consulta = $this->mysqli->query("SELECT * from newblends, imagenes where id = '$idblend' and idCodigoImg = '$idCodImg'");


    if($consulta-> num_rows){
      $fila = $consulta-> fetch_assoc();
      
      $this-> modifBlends = "<form action='$_SERVER[SCRIPT_NAME]' method='post' enctype='multipart/form-data'>\n".
        "<h2>Modifcar Contenido</h2>\n".
        "<input type='text' name='modifNombre' value='".$fila['blend'] . "'>\n".
        "<input type='text' name='modifPrecio' value='".$fila['precio'] . "'>\n".
        "<textarea name='modifDescripcion'>" . $fila['descripcion'] . "</textarea>\n".
        
        "<input type='hidden' name='id' value='".$fila['id'] . "'>\n" .
        "<input type='hidden' name='codImg' value='" . $fila['idCodigoImg'] . "'>\n";
        $this-> modifBlends .= "<img src='". $fila['rutaThumb'] ."' alt='".$fila['nombreThumb']. "' />\n'";
        $this-> modifBlends .= "<input type='file' name='files[]' accept='image/*'>";
        $this-> modifBlends .= "<input name='modificarBoton' type='submit' value='modificar'>\n";
        $this-> modifBlends .= "</form>";   
      }else{        
       /* $this-> traeFormModificar($idblend);*/
      }
  }
  ...
  public function cambiarImagen($id, $nombre, $precio, $descripcion){
    $idImagenUnico = uniqid();
      $update = $this-> mysqli-> query("UPDATE newblends SET blend = '$nombre', precio = '$precio', descripcion = '$descripcion', 
        imagenBlend = '$idImagenUnico' WHERE id = '$id'");
      $this-> altaFotos("fotoGrande/", "thumbnails/", $crearThumbs = true, $idImagenUnico);
...
//=====================================================//  
   ...
   public function armarOptions(){ 
    $consulta = $this->mysqli->query("SELECT * from newblends");
    if($consulta-> num_rows){
      $this-> modifBlends = "<form action='$_SERVER[SCRIPT_NAME]' method='REQUEST'>\n";
      foreach ($consulta as $fila){
        $this-> modifBlends .= "<input class='options' type='radio' name='yourChoice' value='" . $fila['id'] . "'>" . $fila['blend'];
        $this-> modifBlends .= "<br />";
      }
                
      $this-> modifBlends .= "<input class='btndownload' name='downloadMe' type='submit' value='Descargar'>";
      $this-> modifBlends .= "</form>"; 
      }else{        
        echo "me rehuso a entrar";
      }
  }

public function downloadTxT($blendElegido){
  /*$directorio = '/emails';
  $extension = '.csv';
  $coma = ',';
  $enclosed = '"';
  ...
  $blend = $this-> mysqli-> query("SELECT newblends.blend FROM newblends WHERE id=$blendElegido");
  while($resultado = mysqli_fetch_array($blend)){
      $nombre = $resultado['blend'];
  }            
  
  $txt_name = $directorio . "_" . $nombre . "_" . $randomN . $extension;*/
 
 /* $sql = $this-> mysqli-> query("SELECT notificaciones.emailusr 
  FROM notificaciones LEFT JOIN newblends ON notificaciones.idblend = newblends.id WHERE id=$blendElegido
  INTO OUTFILE '$txt_name' FIELDS TERMINATED BY '$coma' ENCLOSED BY '$enclosed' LINES TERMINATED BY '$end'");*/
 
  
  $fh = fopen('emails.txt', 'w');
$result = $this-> mysqli-> query("SELECT notificaciones.emailusr FROM notificaciones LEFT JOIN newblends ON notificaciones.idblend = newblends.id WHERE id=$blendElegido");
...
fclose($fh);
  ...
  echo "Se ha descargado el archivo exitosamente.";
